...
{
    [HttpGet("/tours")]
    public IResponseResult List() => RenderIndex(false);
    
    [HttpGet("/tours/partial")]
    public IResponseResult Partial() => RenderIndex(true);
    ...
    private IResponseResult RenderIndex(bool partial)
{
    ...
    bool Flag(string name) =>
        string.Equals(q[name], "1", StringComparison.OrdinalIgnoreCase);
    ...
    int CategoryRank(Experience e) => e.CategoryName switch
    {
    ...
    if (partial)
        return Page("sem/partials/grid.html", vm); // только грид

    return Page("sem/index.html", vm);            // вся страница
}


    // ===== helpers =====
    private static readonly Regex HtmlRx = new Regex("<.*?>", RegexOptions.Compiled);
    private static string StripHtml(string s) => HtmlRx.Replace(s, " ").Replace("&nbsp;", " ").Trim();
    private static string Shorten(string s, int max) => (s.Length <= max) ? s : s[..max].Trim() + "…";

    // ===== модели таблиц (под имена колонок из init.sql) =====
