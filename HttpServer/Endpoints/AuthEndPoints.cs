...
namespace HttpServer.Endpoints
{
    ...
    internal sealed class AuthEndPoints
    {
        ...
        [HttpGet("/auth")]
        public string LoginPage() => "auth/login.html";

        // современный роут
        [HttpPost("/auth/login")]
        public Task Login(HttpListenerContext ctx) => HandleLogin(ctx);

        // на случай, если форма ещё шлёт на старый адрес
        [HttpPost("/auth/sendEmail")]
        public Task LegacyLogin(HttpListenerContext ctx) => HandleLogin(ctx);
        ...
        private static async Task HandleLogin(HttpListenerContext ctx)
        {
            ...
            // просто уведомляем — у тебя уже есть EmailService
            try
            {
                await EmailService.SendAsync(email, "Login", $"User: {WebUtility.HtmlEncode(email)}");
            }
            catch { /* падать не даём — но и не мешаем логину */ }
            ...
            // НИЧЕГО больше не пишем в тело ответа!
